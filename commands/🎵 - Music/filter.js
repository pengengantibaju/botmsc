...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if(!args[0]) return message.reply({embeds: [
            ...
            .setColor('EF4F4F')
            .setAuthor({name: 'Error', iconURL: 'https://raw.githubusercontent.com/SudhanPlayz/Discord-MusicBot/master/assets/logo.gif'})
            .setDescription(`Please select a suitable filter!
            Name of filters: 3d, bassboost, echo, karaoke, nightcore, vaporwave, flanger, gate, haas, reverb, surround, mcompand, phaser, tremolo, earwax`)
        ]})
        ...
        if(args[0] === 'off' && queue.filter?.length) queue.setFilter(false);
            ...
            .setColor('EF4F4F')
            .setAuthor({name: 'Error', iconURL: 'https://raw.githubusercontent.com/SudhanPlayz/Discord-MusicBot/master/assets/logo.gif'})
            .setDescription('No suitable filter found!')
        ]})
        ...
        message.reply({embeds: [
            ...
            .setDescription(`Change the filter to: **${queue.filters.join(', ') || 'Off'}**`)
        ]})
    }
}
